...
bool qlinear_conv_1x3x3x3_1x3x2x2() {
  ...
  std::vector<uint8_t> weight_values(weight_dims[0] * weight_dims[1]*weight_dims[2] * weight_dims[3]);
  ...
  weight_proto.set_data_type(onnx::TensorProto::UINT8);
  ...
  bias_proto.set_data_type(onnx::TensorProto::UINT8);

  std::vector<int> output_dims = {1, 1, 2, 2}; 
  std::vector<uint8_t> expected_output_values = {
      109, 116, 129, 136
  };
  TensorCreate<uint8_t> expected_output(expected_output_values, output_dims);
  ...
  std::vector<std::variant<int8_t, uint8_t>> x_zero_point = {uint8_t(0)};
  std::vector<std::variant<int8_t, uint8_t>> w_zero_point = {uint8_t(0)};
  std::vector<std::variant<int8_t, uint8_t>> y_zero_point = {uint8_t(0)};
  ...
  q_conv_layer.x_scale = {0.12};
  q_conv_layer.x_zero_point = x_zero_point;
  q_conv_layer.w_scale = {0.14};
  q_conv_layer.w_zero_point = w_zero_point;
  q_conv_layer.y_scale = {0.16};
  q_conv_layer.y_zero_point = y_zero_point;
  q_conv_layer.input_type=onnx::TensorProto::UINT8;
  q_conv_layer.weight_type=onnx::TensorProto::UINT8;


  q_conv_layer.run(tensor_pool);

  Tensor<uint8_t> *out = tensor_pool.get<Tensor<uint8_t> *>(q_conv_layer.outputs.at(0));

  std::vector<uint8_t> out_values = out->get();

  bool result = generate_report("qlinear_conv_1x3x3x3_1x3x2x2", out_values, expected_output_values);
  return result;
}
...
int main(int argc, char *argv[]) {
  ...
  import_array();
  if(!qlinear_conv_1x3x3x3_1x3x2x2()){
    exit(1);
  }
