...
def run_one(args):
  ...
  gati.set_keep_quiet(False); gati.set_arch(config={"sa-arch":args.arch})
  gati.set_remote(args.hostname)
  onnx,gml,res=args.models[0],"/tmp/model.gml",{"status":"fail"}
  try:
    gati.flash(args.bitstreams[0])
...
if __name__=="__main__":
  ...
  p.add_argument('-a','--arch',default='9,4,4'); p.add_argument('-b','--bitstreams', nargs='+', required=True)
