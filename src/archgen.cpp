...
ArchParams Op::Layer::QLinearConv::archgen(const ArchParams &ap) const {
  ...
  ret.im2col_bound_gen_w.v =
      round2pwr(std::log2(this->input_dims.at(0)[TENSOR_4D_HEIGHT]));
...
void write_arch(const ArchParams& ap, const std::string& path) {
...
ArchParams archgen(Op::Graph graph, const std::string& fpga) {
  ...
  if (gbl_args.has_option("archgen-file")) {
    write_arch(ap, gbl_args["archgen-file"].as<std::string>());
    log_info("Wrote hardware macros to {}",gbl_args["archgen-file"].as<std::string>());
  }
...
std::ostream &operator<<(std::ostream &os, const ArchParams &ap) {
  os << "//To Be Generated by gaticc, do not edit manually\n";
