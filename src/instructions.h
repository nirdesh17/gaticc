...
#define EltWise_LeftOperandStartAddress_LOW 8
#define EltWise_LeftOperandStartAddress_HIGH 39
#define EltWise_LeftOperandStartAddress_COUNT 32
#define EltWise_LeftOperandEndAddress_LOW 40
#define EltWise_LeftOperandEndAddress_HIGH 71
#define EltWise_LeftOperandEndAddress_COUNT 32
#define EltWise_RightOperandStartAddress_LOW 72
#define EltWise_RightOperandStartAddress_HIGH 103
#define EltWise_RightOperandStartAddress_COUNT 32
#define EltWise_RightOperandEndAddress_LOW 104
#define EltWise_RightOperandEndAddress_HIGH 135
...
inline Table get_conv_table(const std::bitset<INST_SIZE_BITS> &inst) {
  Table tbl;
  tbl.tbl.insert({"Opcode", bitset_range_get<CONV_Opcode_COUNT, INST_SIZE_BITS>(
                                inst, CONV_Opcode_LOW, CONV_Opcode_HIGH)});
  tbl.order.push_back("Opcode");
  tbl.tbl.insert({"IW", bitset_range_get<CONV_IW_COUNT, INST_SIZE_BITS>(
                            inst, CONV_IW_LOW, CONV_IW_HIGH)});
  tbl.order.push_back("IW");
  tbl.tbl.insert({"IH", bitset_range_get<CONV_IH_COUNT, INST_SIZE_BITS>(
                            inst, CONV_IH_LOW, CONV_IH_HIGH)});
  tbl.order.push_back("IH");
  tbl.tbl.insert({"OW", bitset_range_get<CONV_OW_COUNT, INST_SIZE_BITS>(
                            inst, CONV_OW_LOW, CONV_OW_HIGH)});
  tbl.order.push_back("OW");
  tbl.tbl.insert({"OH", bitset_range_get<CONV_OH_COUNT, INST_SIZE_BITS>(
                            inst, CONV_OH_LOW, CONV_OH_HIGH)});
  tbl.order.push_back("OH");
  tbl.tbl.insert({"IC", bitset_range_get<CONV_IC_COUNT, INST_SIZE_BITS>(
                            inst, CONV_IC_LOW, CONV_IC_HIGH)});
  tbl.order.push_back("IC");
  tbl.tbl.insert({"KN", bitset_range_get<CONV_KN_COUNT, INST_SIZE_BITS>(
                            inst, CONV_KN_LOW, CONV_KN_HIGH)});
  tbl.order.push_back("KN");
  tbl.tbl.insert({"KW", bitset_range_get<CONV_KW_COUNT, INST_SIZE_BITS>(
                            inst, CONV_KW_LOW, CONV_KW_HIGH)});
  tbl.order.push_back("KW");
  tbl.tbl.insert({"KH", bitset_range_get<CONV_KH_COUNT, INST_SIZE_BITS>(
                            inst, CONV_KH_LOW, CONV_KH_HIGH)});
  tbl.order.push_back("KH");
  tbl.tbl.insert({"Stride", bitset_range_get<CONV_Stride_COUNT, INST_SIZE_BITS>(
                                inst, CONV_Stride_LOW, CONV_Stride_HIGH)});
  tbl.order.push_back("Stride");
  tbl.tbl.insert({"Pad", bitset_range_get<CONV_Pad_COUNT, INST_SIZE_BITS>(
                             inst, CONV_Pad_LOW, CONV_Pad_HIGH)});
  tbl.order.push_back("Pad");
  tbl.tbl.insert(
      {"PadSides", bitset_range_get<CONV_PadSides_COUNT, INST_SIZE_BITS>(
                       inst, CONV_PadSides_LOW, CONV_PadSides_HIGH)});
...
inline Table get_tailblock_table(const std::bitset<INST_SIZE_BITS> &inst) {
  ...
  tbl.order.push_back("Opcode");
  tbl.tbl.insert(
      {"BNEn", bitset_range_get<TailBlock_BNEn_COUNT, INST_SIZE_BITS>(
  ...
  tbl.tbl.insert(
      {"ActEn", bitset_range_get<TailBlock_ActEn_COUNT, INST_SIZE_BITS>(
  ...
  tbl.tbl.insert(
      {"PoolCeil", bitset_range_get<TailBlock_PoolCeil_COUNT, INST_SIZE_BITS>(
                       inst, TailBlock_PoolCeil_LOW, TailBlock_PoolCeil_HIGH)});
  tbl.order.push_back("PoolCeil");
  tbl.tbl.insert(
      {"PoolModCount",
       bitset_range_get<TailBlock_PoolModCount_COUNT, INST_SIZE_BITS>(
           inst, TailBlock_PoolModCount_LOW, TailBlock_PoolModCount_HIGH)});
  tbl.order.push_back("PoolModCount");
  tbl.tbl.insert(
      {"PoolPadSides",
       bitset_range_get<TailBlock_PoolPadSides_COUNT, INST_SIZE_BITS>(
           inst, TailBlock_PoolPadSides_LOW, TailBlock_PoolPadSides_HIGH)});
  ...
  tbl.tbl.insert(
      {"BiasWidth",
       bitset_range_get<TailBlock_BiasWidth_COUNT, INST_SIZE_BITS>(
           inst, TailBlock_BiasWidth_LOW, TailBlock_BiasWidth_HIGH)});
...
inline Table get_outputblock_table(const std::bitset<INST_SIZE_BITS> &inst) {
  ...
  tbl.tbl.insert(
      {"DispatchEn",
       bitset_range_get<OutputBlock_DispatchEn_COUNT, INST_SIZE_BITS>(
           inst, OutputBlock_DispatchEn_LOW, OutputBlock_DispatchEn_HIGH)});
  tbl.order.push_back("DispatchEn");
  tbl.tbl.insert(
      {"DispatchID",
       bitset_range_get<OutputBlock_DispatchID_COUNT, INST_SIZE_BITS>(
           inst, OutputBlock_DispatchID_LOW, OutputBlock_DispatchID_HIGH)});
  tbl.order.push_back("DispatchID");
  tbl.tbl.insert(
      {"OnChipAcc",
       bitset_range_get<OutputBlock_OnChipAcc_COUNT, INST_SIZE_BITS>(
           inst, OutputBlock_OnChipAcc_LOW, OutputBlock_OnChipAcc_HIGH)});
...
inline Table get_fc_table(const std::bitset<INST_SIZE_BITS> &inst) {
  Table tbl;
  tbl.tbl.insert({"Opcode", bitset_range_get<FC_Opcode_COUNT, INST_SIZE_BITS>(
  ...
  tbl.tbl.insert(
      {"InputRows", bitset_range_get<FC_InputRows_COUNT, INST_SIZE_BITS>(
                        inst, FC_InputRows_LOW, FC_InputRows_HIGH)});
  tbl.order.push_back("InputRows");
  tbl.tbl.insert({"DropoutConstant",
  ...
  tbl.tbl.insert({"Flatten", bitset_range_get<FC_Flatten_COUNT, INST_SIZE_BITS>(
                                 inst, FC_Flatten_LOW, FC_Flatten_HIGH)});
  tbl.order.push_back("Flatten");
  tbl.tbl.insert(
      {"ImageDim", bitset_range_get<FC_ImageDim_COUNT, INST_SIZE_BITS>(
...
inline Table get_start_table(const std::bitset<INST_SIZE_BITS> &inst) {
  Table tbl;
  tbl.tbl.insert(
      {"Opcode", bitset_range_get<START_Opcode_COUNT, INST_SIZE_BITS>(
                     inst, START_Opcode_LOW, START_Opcode_HIGH)});
  tbl.order.push_back("Opcode");
  tbl.tbl.insert({"LayerNumber",
...
inline Table get_nms_table(const std::bitset<INST_SIZE_BITS> &inst) {
  Table tbl;
  tbl.tbl.insert({"Opcode", bitset_range_get<NMS_Opcode_COUNT, INST_SIZE_BITS>(
                                inst, NMS_Opcode_LOW, NMS_Opcode_HIGH)});
  tbl.order.push_back("Opcode");
  tbl.tbl.insert({"IOU", bitset_range_get<NMS_IOU_COUNT, INST_SIZE_BITS>(
                             inst, NMS_IOU_LOW, NMS_IOU_HIGH)});
  tbl.order.push_back("IOU");
  tbl.tbl.insert(
      {"ScoreThresh", bitset_range_get<NMS_ScoreThresh_COUNT, INST_SIZE_BITS>(
                          inst, NMS_ScoreThresh_LOW, NMS_ScoreThresh_HIGH)});
  tbl.order.push_back("ScoreThresh");
  tbl.tbl.insert(
      {"TotalInBoxes", bitset_range_get<NMS_TotalInBoxes_COUNT, INST_SIZE_BITS>(
                           inst, NMS_TotalInBoxes_LOW, NMS_TotalInBoxes_HIGH)});
  tbl.order.push_back("TotalInBoxes");
  tbl.tbl.insert(
      {"MaxOutBoxes", bitset_range_get<NMS_MaxOutBoxes_COUNT, INST_SIZE_BITS>(
                          inst, NMS_MaxOutBoxes_LOW, NMS_MaxOutBoxes_HIGH)});
  tbl.order.push_back("MaxOutBoxes");
  tbl.tbl.insert(
      {"CornerCord", bitset_range_get<NMS_CornerCord_COUNT, INST_SIZE_BITS>(
                         inst, NMS_CornerCord_LOW, NMS_CornerCord_HIGH)});
  tbl.order.push_back("CornerCord");
  tbl.tbl.insert(
      {"TotalClasses", bitset_range_get<NMS_TotalClasses_COUNT, INST_SIZE_BITS>(
                           inst, NMS_TotalClasses_LOW, NMS_TotalClasses_HIGH)});
  tbl.order.push_back("TotalClasses");
  tbl.tbl.insert(
      {"BoxStartAddr", bitset_range_get<NMS_BoxStartAddr_COUNT, INST_SIZE_BITS>(
                           inst, NMS_BoxStartAddr_LOW, NMS_BoxStartAddr_HIGH)});
  tbl.order.push_back("BoxStartAddr");
  tbl.tbl.insert(
      {"BoxEndAddr", bitset_range_get<NMS_BoxEndAddr_COUNT, INST_SIZE_BITS>(
                         inst, NMS_BoxEndAddr_LOW, NMS_BoxEndAddr_HIGH)});
  tbl.order.push_back("BoxEndAddr");
  tbl.tbl.insert({"ScoreStartAddr",
                  bitset_range_get<NMS_ScoreStartAddr_COUNT, INST_SIZE_BITS>(
                      inst, NMS_ScoreStartAddr_LOW, NMS_ScoreStartAddr_HIGH)});
  tbl.order.push_back("ScoreStartAddr");
  tbl.tbl.insert(
      {"ScoreEndAddr", bitset_range_get<NMS_ScoreEndAddr_COUNT, INST_SIZE_BITS>(
                           inst, NMS_ScoreEndAddr_LOW, NMS_ScoreEndAddr_HIGH)});
...
inline Table get_eltwise_table(const std::bitset<INST_SIZE_BITS> &inst) {
  Table tbl;
  tbl.tbl.insert(
      {"Opcode", bitset_range_get<EltWise_Opcode_COUNT, INST_SIZE_BITS>(
                     inst, EltWise_Opcode_LOW, EltWise_Opcode_HIGH)});
  tbl.order.push_back("Opcode");
  tbl.tbl.insert(
      {"EltType", bitset_range_get<EltWise_EltType_COUNT, INST_SIZE_BITS>(
                      inst, EltWise_EltType_LOW, EltWise_EltType_HIGH)});
  tbl.order.push_back("EltType");
  tbl.tbl.insert(
      {"LeftOperandStartAddress",
       bitset_range_get<EltWise_LeftOperandStartAddress_COUNT, INST_SIZE_BITS>(
           inst, EltWise_LeftOperandStartAddress_LOW,
           EltWise_LeftOperandStartAddress_HIGH)});
  tbl.order.push_back("LeftOperandStartAddress");
  tbl.tbl.insert(
      {"LeftOperandEndAddress",
       bitset_range_get<EltWise_LeftOperandEndAddress_COUNT, INST_SIZE_BITS>(
           inst, EltWise_LeftOperandEndAddress_LOW,
           EltWise_LeftOperandEndAddress_HIGH)});
  tbl.order.push_back("LeftOperandEndAddress");
  tbl.tbl.insert(
      {"RightOperandStartAddress",
       bitset_range_get<EltWise_RightOperandStartAddress_COUNT, INST_SIZE_BITS>(
           inst, EltWise_RightOperandStartAddress_LOW,
           EltWise_RightOperandStartAddress_HIGH)});
  tbl.order.push_back("RightOperandStartAddress");
  tbl.tbl.insert(
      {"RightOperandEndAddress",
       bitset_range_get<EltWise_RightOperandEndAddress_COUNT, INST_SIZE_BITS>(
           inst, EltWise_RightOperandEndAddress_LOW,
