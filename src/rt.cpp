...
/* Just a thin wrapper to make logging common for all layers */
template <typename T>
static Tensor<T>* rah_read(BinBlob &blob, Rah &rah, const Op::LayerBase *l, uint32_t expected_data_size, uint32_t expected_hash) {
...
template <typename T>
static void post_read(const Op::LayerBase *l, TensorPool &tensor_pool, Tensor<T>* tensor) {
  if (tensor_pool.has_value(l->outputs.at(0))) {
    tensor_pool.free(l->outputs.at(0));
  }
...
void Op::Layer::QLinearConv::receive_output(TensorPool &tensor_pool, Rah &rah) const {
  uint32_t expected_hash = string_hash(this->name);
  uint32_t expected_data_size = aligned_conv_output(this->pipelined_output_dims) * Op::tpdt_sizeof(this->output_type.at(0));
...
void Op::Layer::QGemm::receive_output(TensorPool &tensor_pool, Rah &rah) const {
  uint32_t expected_hash = string_hash(this->name);
  uint32_t expected_data_size = aligned_fc_io(&this->output_dims[0]) * Op::tpdt_sizeof(this->output_type[0]);
  if (this->output_type[0] == onnx::TensorProto_DataType_INT8) {
...
void Op::Layer::Transpose::receive_output(TensorPool &tensor_pool, Rah &rah) const {
  uint32_t expected_hash = string_hash(this->name);
  uint32_t expected_data_size = ceil_mod(prod(this->output_dims.at(0)) * Op::tpdt_sizeof(this->output_type.at(0)), WORD_SIZE);
  if (this->output_type[0] == onnx::TensorProto_DataType_INT8) {
    va_receive<int8_t>(rah, tensor_pool, this, expected_data_size, expected_hash);
  } else if (this->output_type[0] == onnx::TensorProto_DataType_UINT8) {
    va_receive<uint8_t>(rah, tensor_pool, this, expected_data_size, expected_hash);
...
void Op::Layer::QLinearSigmoid::receive_output(TensorPool &tensor_pool, Rah &rah) const {
  uint32_t expected_hash = string_hash(this->name);
  uint32_t expected_data_size = aligned_qle(this->pipelined_output_dims).at(0) * Op::tpdt_sizeof(this->output_type.at(0));
  if (this->output_type[0] == onnx::TensorProto_DataType_INT8) {
    eltwise_receive<int8_t>(rah, tensor_pool, this, expected_data_size, expected_hash);
...
template <typename T>
static void sa_align_input_eltwise(BinBlob &blob,
                                   const Op::Layer::QLinearEltwise *l,
  ...
  IVec2D og_dims_v{tensor->get_dims()};
  auto aligned_dims =
      aligned_conv_input_dims(og_dims_v, l->constant_data->dims())[0];
  std::vector<int> aligned_dims_vec(aligned_dims.begin(), aligned_dims.end());
  auto sa_arch = get_sa_arch();
  int chan_dim = sa_arch[SA_ARCH_N];
  sa_align_input_aux(blob, data_size, addr, tensor, aligned_dims, chan_dim);
  tt.stop();
  log_info("SA Align time: {} us\n", tt.difference().count());
}
...
template <typename T>
static void
sa_send_input_eltwise(const Op::LayerBase *l, TensorPool &tensor_pool,
                      AddressGen &generator, Rah &rah, IOAddrTbl &tbl) {
  Tensor<T> *input_tensor = tensor_pool.get<Tensor<T> *>(l->inputs.at(0));
  auto ireg = tbl.at(l->name).first.at(0);
  uint32_t addr = generator.io_addr_from_register(ireg);
  log_info("sending input for register {}, addr is {}\n", ireg, addr);
  ...
  auto dims = input_tensor->get_dims();
  IVec2D dims_wrapper = {dims};
  uint32_t og_aligned_size =
      aligned_conv_input(dims_wrapper, cc->constant_data->dims()) * sizeof(T);
  uint32_t total_size_with_packets = io_tensor_packet_size(og_aligned_size);
  BinBlob blob(total_size_with_packets);
  sa_align_input_eltwise<T>(blob, cc, og_aligned_size, addr, input_tensor);
  blob.append_dwp_header(0, 0);
  if (get_verbose()) {
    blob.write("input_data.bin");
  }
...
void Op::Layer::QLinearEltwise::send_input(TensorPool &tensor_pool,
                                           AddressGen &generator, Rah &rah,
                                           IOAddrTbl &tbl) const {
  if (input_type.at(0) == onnx::TensorProto_DataType_INT8) {
    sa_send_input_eltwise<int8_t>(this, tensor_pool, generator, rah, tbl);
  } else if (input_type.at(0) == onnx::TensorProto_DataType_UINT8) {
    sa_send_input_eltwise<uint8_t>(this, tensor_pool, generator, rah, tbl);
  } else {
    log_fatal("QLinearConv::send_input() can't handle {} type",
              get_tensorproto_dtype_name(input_type.at(0)));
  }
}
