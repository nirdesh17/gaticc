...
template <typename inputT, typename outputT> bool ConvTransformer<inputT, outputT>::is_lsfe(Point const &index) {
  int y = index.second;
  return (y == (m_cp.imap[WIDTH]+m_cp.pad[LEFT]+m_cp.pad[RIGHT] - m_cp.k[WIDTH])) ? true : false;
}
...
template <typename inputT, typename outputT> bool ConvTransformer<inputT, outputT>::is_lsle(Point const &index) {
  int y = index.second;
  return (y == m_cp.imap[WIDTH]+m_cp.pad[LEFT]+m_cp.pad[RIGHT] - 1);
}
...
bool ConvTransformer<inputT, outputT>::has_occured(Point const &p,
  ...
  int y = p.second;
  int lsfe = (m_cp.imap[WIDTH]+m_cp.pad[LEFT]+m_cp.pad[RIGHT] - m_cp.k[WIDTH]);
...
template <typename inputT, typename outputT> bool ConvTransformer<inputT, outputT>::is_last_kernel(Point const &p) {
  return ((p.first == (m_cp.imap[HEIGHT]+m_cp.pad[UP]+m_cp.pad[DOWN] - m_cp.k[HEIGHT])) &&
          (p.second == (m_cp.imap[WIDTH]+m_cp.pad[LEFT]+m_cp.pad[RIGHT] - m_cp.k[WIDTH])));
}
