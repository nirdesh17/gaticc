...
PE::PE(int id, weight_t w, reg_t r, reg_t input_buffer):
    id {id},
    weight {w}, reg {r}, input_buffer {input_buffer}, 
    aux_buffer {0}, ps_buffer {0} {
};
...
weight_t PE::get_weight() {
    return this->weight;
}
reg_t PE::get_reg() {
   return this->reg;
} 
reg_t PE::get_input_buffer() {
    return this->input_buffer;
}
reg_t PE::get_aux_buffer() {
    return this->aux_buffer;
}
reg_t PE::get_ps_buffer() {
    return this->ps_buffer;
}
void PE::set_input_buffer(reg_t r) {
    this->input_buffer = r;
}
void PE::set_weight(weight_t w) {
    this->weight = w;
}
void PE::set_reg(reg_t r) {
    this->reg = r;
}
void PE::set_aux_buffer(reg_t r) {
    this->aux_buffer = r;
}
void PE::set_ps_buffer(reg_t r) {
    this->ps_buffer = r;
}
reg_t PE::mac() {
...
void PE::print_pe() {
    printf("Id: %d\tWeight: %d\tReg: %d\tInput: %d\tAux: %d\tPs: %d\n", 
            id, weight, reg, input_buffer, aux_buffer, ps_buffer);
}
...
SA::SA(int r, int c, bool profile_enabled): rows{r}, columns{c}, 
    profile_enabled {profile_enabled}, output_array{Mat(columns)},
...
PE& SA::get_pe_from_vertex(PE_Graph::Vertex& v) {
    return g[v];
}
...
PE_Graph::Vertex& SA::get_vertex_from_adjacency_iterator(PE_Graph::Adjacency_iterator& itr) {
...
PE_Graph::Neighbours SA::assign_vertices(const PE_Graph::Neighbours& neighbours, PE_Graph::Vertex& v) {
...
bool SA::is_output_vertex(PE_Graph::Vertex& v) {
    ...
    if (index >= (vertarray.size()-columns) && index <= (vertarray.size()-1)) {
        return true;
    }
    return false;
}
...
void SA::print_array() {
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < columns; ++j) {
            printf("%d\t", get_pe_from_vertex(vertarray[i*columns+j]).get_weight());
#if 0
...
void Tree::connect_btree(std::pair<int,int> xy) {
    ...
    if (is_node_allowed(left_node)) {
        if ((std::find_if(edge_memo.begin(), edge_memo.end(), 
                    [&xy, &left_node](auto p) { return xy == p.first && left_node == p.second; }))
                == edge_memo.end()) {
            if ((std::find_if(child_memo.begin(), child_memo.end(), 
                ...
                edge_memo.push_back(std::pair<std::pair<int,int>, std::pair<int,int>>(xy, left_node));
    ...
    if (is_node_allowed(right_node)) {
        if ((std::find_if(edge_memo.begin(), edge_memo.end(), 
                    [&xy, &right_node](auto p) { return xy == p.first && right_node == p.second; }))
                == edge_memo.end()) {
            if ((std::find_if(child_memo.begin(), child_memo.end(), 
                        [&right_node](auto p) { return p == right_node; }))
                    == child_memo.end()) {
                boost::add_edge(vertex_map.at(xy), vertex_map.at(right_node), g);
                edge_memo.push_back(std::pair<std::pair<int,int>, std::pair<int,int>>(xy, right_node));
...
void Profiler::incr_cycles(int count) {
   cycles += count; 
}

uint64_t Profiler::get_cycles() {
    return cycles;
}

int Chainblock::exec(int x) {
    return x;
}
...
void Chain::push(Chainblock *b) {
    arr.push_back(b);
}

Chainblock* Chain::at(int i) {
    return arr.at(i);
}
