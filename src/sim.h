...
class Pooler {
  static fMat movement(Mat &input, int ip_rows, int ip_columns, int stride,
                       int padding, int dilation, int kernel_rows,
                       int kernel_cols, action);
  static float max_pooler_action(std::vector<int> &input, int ip_rows,
                                 int ip_columns, int stride, int padding,
                                 int dilation, int kernel_rows,
                                 int kernel_cols);
  static float average_pooler_action(std::vector<int> &input, int ip_rows,
                                     int ip_columns, int stride, int padding,
                                     int dilation, int kernel_rows,
                                     int kernel_cols);

public:
  static fMat max_pooler(Mat &input, int ip_rows, int ip_columns, int stride,
                         int padding, int dilation, int kernel_rows,
                         int kernel_cols);
  static fMat average_pooler(Mat &input, int ip_rows, int ip_columns,
                             int stride, int padding, int dilation,
                             int kernel_rows, int kernel_cols);
  static fMat global_average_pooler(Mat &input, int ip_rows, int ip_columns,
                                    int stride, int padding, int dilation,
                                    int kernel_rows, int kernel_cols);
};
...
namespace PE_Graph {
using Graph =
    boost::adjacency_list<boost::vecS, boost::listS, boost::directedS, PE>;
...
private:
  ...
  PE_Graph::Vertex &
  get_vertex_from_adjacency_iterator(PE_Graph::Adjacency_iterator &itr);
  ...
  PE_Graph::Neighbours assign_vertices(const PE_Graph::Neighbours &neighbours,
                                       PE_Graph::Vertex &v);
