...
std::vector<Mat> SASA::input_tensor_transformer(std::vector<Mat> input_tensor,std::vector<ConvTransformer*> CT_ptr){
	...
	for(int i = 0 ; i < input_tensor.size() ; i ++){
		...
		transformed_mats.push_back( CT_ptr.at(i)->transform(temp_vec)); 
	}
...
Mat SASA::operatorr(Mat transformed_mats, SA* SA_ptr, ConvTransformer* CT_ptr){
	...
	for(int i = 0 ; i < sa_channels; i ++){
			...
			SA_ptr->propagate(transformed_mats,c1);  // fix chain here
...
void SASA::splitter(std::vector<Mat>&vec , Mat temp_mat , int channel_number , int kernel_number, int input_kernel_size){
	for(int i = kernel_number ; i < (kernel_number + temp_mat.size()) ; i ++){

		if(i>=input_kernel_size){
			break;
		}

		vec.at(channel_number).push_back( temp_mat.at(i));   // fix
	}
}

void SASA::master(std::vector<Mat> input_tensor,std::vector<std::vector<Mat>> input_kernel){  //NCHW
...
int channel_count =  input_kernel.at(0).size();  // sa_channels*(input_kernel.at(0).size()/sa_channels) + (input_kernel.at(0).size() % sa_channels);
	...
	for(int k = 0 ;  k < input_kernel.at(0).size(); k ++ ){
		for( int i = 0 ; i < input_kernel.size(); i ++, channel_count-=sa_channels){
