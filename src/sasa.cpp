...
std::vector<ConvTransformer*> SASA::create_ConvTransformer(int IW, int IH, int KW, int KH, int srows, int scols, int input_tensor_channels){
...
std::vector<Mat> SASA::input_tensor_transformer(std::vector<Mat>& input_tensor,std::vector<ConvTransformer*> CT_ptr){
	...
	for(int i = 0 ; i < input_tensor.size() ; i ++){
		temp_vec=mat2v<int,int>(input_tensor.at(i), input_tensor.at(i).size(),input_tensor.at(i).at(i).size());
...
std::vector<int> SASA:: load_kernel_tensors(std::vector<std::vector<Mat>>& input_kernel, int kernel_channel,int kernel_number){
	...
	std::vector<int> zero_vec( input_kernel.at(0).at(0).size() * input_kernel.at(0).at(0).at(0).size() ,0);
...
void SASA::load_weights_tensor(SA* SA_ptr, ConvTransformer* CT_ptr, std::vector<int>& input) {
	std::vector<int> temp_vec;
	temp_vec = CT_ptr->transform_weights(input,sa_channel_rows,sa_channel_columns);
...
void SASA::splitter(std::vector<Mat>& vec , Mat& temp_mat , int channel_number , int kernel_number, int input_kernel_size){  // here too
...
Mat SASA::master(std::vector<Mat>& input_tensor,std::vector<std::vector<Mat>>& input_kernel){  //NCHW
	
	std::vector<SA*> SA_ptr = create_sasa(sa_channel_rows,sa_channel_columns,sa_channels);
	std::vector<Mat> transformed_mats;
	std::vector<int> output_weights;
	std::vector<Mat> vec(input_kernel.at(0).size());   // channel pointers -> kernel pointers (0-7) -> linear upto c0k7
	Mat temp_mat;
	Mat output_mat;
	std::vector<ConvTransformer*> CT_ptr= create_ConvTransformer(input_tensor.at(0).size(),input_tensor.at(0).at(0).size(),
																...
																sa_channel_rows,sa_channel_columns,input_tensor.size());
...
*  SAs with same channels but different kernel and then after all the kernels are done for the ongoing set of channels....
* channels are updated and then the process is repeated all over again.
*/
int channel_count =  input_kernel.at(0).size();  // sa_channels*(input_kernel.at(0).size()/sa_channels) + (input_kernel.at(0).size() % sa_channels);
	
	
	for(int k = 0 ;  k < input_kernel.at(0).size()/sa_channels +1; k ++, (channel_count>sa_channels ?channel_count-=sa_channels:channel_count) ){
		for( int i = 0 ; i < input_kernel.size(); i ++){
			for( int j = 0 ; j < (channel_count<= sa_channels?channel_count:sa_channels); j++){
				...
				temp_mat = operatorr(transformed_mats.at(k*sa_channels + j),SA_ptr.at(j), CT_ptr.at(j));
				splitter(vec, temp_mat, (k*sa_channels+j), i*8 , input_kernel.size());
