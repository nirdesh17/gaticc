...
def postprocess(inference_output, frame, height, width):
    ...
    print("After NMS:", len(nboxes), len(nclasses), len(nscores))
    ...
    return draw(frame, boxes, scores, classes, height, width)
...
if __name__ == '__main__':
    ...
    onnx_path = "../../onnx/yolov8n_quantized_nonms_mAP_20_3.onnx"
    bitstream = "rah.hex"
    gml_path = "model.gml"
    ...
    gati.load(onnx_path, gml_path,"verbose")
    ...
    if args.image:
        ...
        while True:
            ...
            outputs = new_outputs
            frame = postprocess(outputs, image, height, width)
    ...
    for idx, fname in enumerate(files_to_process):
        ...
        outputs = new_outputs
        frame = postprocess(outputs, image, height, width)
        cv2.imwrite(f'output_{fname}', frame)
