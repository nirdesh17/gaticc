...
def align_dims(input_dims, sa_arch, dk):
    new_dims = [ceil_mod(input_dims[0], sa_arch[2]), ceil_mod(input_dims[1], dk), ceil_mod(input_dims[2], dk)]
    return new_dims
...
def viz_sa_input(input_dims, sa_arch, dram_width):
    ...
    for c in range(new_dims[0] // sa_arch[2]):
        for hi in range(new_dims[1]):
            for wi in range(new_dims[2] // dk):
                row = []
                for ci in range(sa_arch[2]):
                    for ei in range(dk):
                        ...
                        h = hi
                        w = wi * dk + ei
                        if chan >= input_dims[0] or h >= input_dims[1] or w >= input_dims[2]:
                            row.append("0")
                        else:
                            row.append(f"c{chan}h{h}w{w}")
