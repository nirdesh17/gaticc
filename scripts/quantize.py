...
def raw_load_images(images_folder: str, height: int, width: int, size_limit=0):
    """
    ...
    print(f"raw batch data shape {batch_data.shape}")
    return batch_data
...
class RawDataReader(CalibrationDataReader):
    def __init__(self, calibration_image_folder: str, model_path: str):
        ...
        print(f"RawDataReader w,h {width}, {height}")
        # Convert image to input data
        self.nhwc_data_list = raw_load_images(
            calibration_image_folder, height, width, size_limit=0
        )
        self.input_name = session.get_inputs()[0].name
        self.datasize = len(self.nhwc_data_list)
